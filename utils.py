...
def get_office_time():
    """Returns the current datetime in the configured US Timezone."""
    tz = pytz.timezone(config.TIMEZONE)
    return datetime.now(tz)

def get_time_based_greeting():
    """Returns 'good morning', 'good afternoon', or 'good evening' based on office time."""
    now = get_office_time()
    hour = now.hour
    if 5 <= hour < 12:
        return "good morning"
...
def parse_user_date(date_string: str) -> str:
    ...
    settings = {
        ...
        'RELATIVE_BASE': get_office_time().replace(tzinfo=None) 
    }
